...
**PR #7 Cost-Summary Endpoint + Harness 12 Assertions (additive scope from PR #6, IN-FLIGHT 2026-05-07):** Branch `feat/pr7-cost-ledger-additive` rebased onto merged-main `bcf2eb3` — 4 commits ahead (Karl Phase F `45855e5` + Brandy R3-style fix `100e4aa` + Brandy Phase G additive `bcd1bc1` + this docs commit). New `GET /api/clients/:clientId/cost-summary?month=YYYY-MM&breakdown=event_type|source` route in `os-api/src/index.ts` (Karl Phase F); `getCostSummaryForClient(clientId, { month, breakdown })` helper in `os-api/src/db.ts` returning aggregated `totalUsd`, `entryCount`, `breakdown` rows (sorted by totalUsd descending), and most-frequent `rateCardVersion` from `cost_ledger_entries` rows in the [YYYY-MM-01, YYYY-(MM+1)-01) range using the existing `(client_id, created_at DESC)` Migration 017 index. Auth gate: `jwtAuthEnabled compute → verifier (only when flag on per PR #6 R1-1) → 401 if missing-and-flag-on → tenant 403 if mismatch (FIRES BEFORE getClient lookup per PR #7 R3-style fix to close existence-leak class same as PR #6 R3-1) → getClient lookup → 404 → cost-summary helper`. Defensive query param validation: `month` regex `^\d{4}-(0[1-9]|1[0-2])$`, `breakdown` strict enum `event_type | source`. **Phase G additive**: harness `_pr4-storage-and-ledger.ts` extended in place from 10 to 12 assertions — Assertion 11 cross-tenant cost-summary returns HTTP 403 (validates the existence-leak fix); Assertion 12 own-tenant cost-summary returns HTTP 200 with non-zero `totalUsd`, non-empty `breakdown`, `entryCount >= 1` ($0.0123 / 1 entry / 1 bucket from seeded ledger). Helpers added: `requestCostSummary` + `currentYearMonth()` UTC-based `YYYY-MM`. Final banner: `12/12 — PR #4 + PR #5 + PR #6 + PR #7 verified`. Validation 12/12 against os-api PID 96670 from `~/proto_front-pr7` worktree with `JWT_AUTH_ENABLED=true` post-rebase. **Phase E (Cohere/Gemini Embedding cost emission) DEFERRED to PR #8** — surfaced brief discrepancy: brief calls Cohere usage "v4 reranker" with rerank pricing, but `brand-engine/brand_engine/core/embeddings.py` actually uses Cohere v4 EMBEDDINGS (`cohere_client.embed(...)`); zero `rerank` call sites in the codebase. PR #8 needs brief amendment with corrected Cohere Embed v4 token-based pricing model + architecture decision (return-tuple vs instance-state vs direct-DB writes from brand-engine). **Phase D (cost-ledger LIMIT push-to-DB + fire-and-forget recordCost) also deferred to PR #8** — P2 perf, not billing-critical. PR #7 ships F + G additive only as a focused PR. Brief: `~/agent-vault/briefs/2026-05-06-pr7-cost-ledger-hardening-additive.md`.
