...
**Storage:** `artifacts` bucket for generated images/videos. **Bucket `public = false` since
PR #5 Migration 018 (2026-05-04)** — HUD readers migrated to signed URLs in lockstep
(`GET /api/artifacts/:id/signed-url` endpoint with 1h TTL + JWT-tenant gate). Write-side
governed by client-scoped RLS policies on `storage.objects` since PR #4 Migration 016 —
INSERT/UPDATE/DELETE require `(storage.foldername(name))[1] = jwt_client_id() OR auth.role() = 'service_role'`.
SELECT policy from Migration 016 now ACTIVE (was dormant while bucket public; engaged on
`/object/sign/` + `/object/authenticated/` URL patterns post-018). `/object/public/` pattern
returns HTTP 400 — exfiltrated URLs no longer fetch bytes.
Optional Cloudinary CDN dual-write for platform-specific variants (10 presets).

**Migrations:** `supabase/migrations/001-018` — all applied via `supabase db query --linked < migrations/NNN_*.sql` with project-scoped PAT (007 via Management API 2026-04-17; 013 2026-05-02; 014+015 2026-05-03 for Phase 7 multi-tenant RLS — see PR #3; 016+017 2026-05-04 for PR #4 Storage write-side RLS + cost ledger; 018 2026-05-04 for PR #5 Storage read-side hardening).

**PR #7 Cost-Summary Endpoint + Harness 13 Assertions (additive scope from PR #6, MERGED 2026-05-07T22:39:19Z squash `d2875a6`):** Per-client monthly cost-summary endpoint shipped end-to-end. New `GET /api/clients/:clientId/cost-summary?month=YYYY-MM&breakdown=event_type|source` route in `os-api/src/index.ts` (Karl Phase F); `getCostSummaryForClient(clientId, { month, breakdown })` helper in `os-api/src/db.ts` returning aggregated `totalUsd`, `entryCount`, `breakdown` rows (sorted by totalUsd descending with deterministic `localeCompare` tie-break per CR R1-1), and most-frequent `rateCardVersion` (sorted by count desc + version asc per CR R1-1) from `cost_ledger_entries` rows in the [YYYY-MM-01, YYYY-(MM+1)-01) range using the existing `(client_id, created_at DESC)` Migration 017 index. Auth gate (auth-first per CR R1-2): `jwtAuthEnabled compute → verifier (only when flag on per PR #6 R1-1) → 401 if missing-and-flag-on → tenant 403 if mismatch (FIRES BEFORE getClient lookup per PR #7 R3-style fix to close existence-leak class same as PR #6 R3-1) → query param parse + regex/enum validation → 400 → getClient lookup → 404 → cost-summary helper`. Defensive query param validation: `month` regex `^\d{4}-(0[1-9]|1[0-2])$`, `breakdown` strict enum `event_type | source`. **Phase G additive (post-CR-R1)**: harness `_pr4-storage-and-ledger.ts` extended in place from 10 to 13 assertions — Assertion 11 missing-JWT 401 (mirrors 8/9 — guards auth-order regression per CR R1-3), Assertion 12 cross-tenant 403 (validates existence-leak fix), Assertion 13 own-tenant 200 with non-zero `totalUsd`, non-empty `breakdown`, `entryCount >= 1` ($0.0123 / 1 entry / 1 bucket from seeded ledger). Helpers added: `requestCostSummary` + `currentYearMonth(d?)` UTC-based `YYYY-MM`. Final banner: `13/13 — PR #4 + PR #5 + PR #6 + PR #7 verified`. **CR convergence on PR branch:** R1 3 findings (R1-1 non-deterministic ordering / R1-2 auth gate before query param validation / R1-3 missing 401 assertion) → fix `e5627ac` (1 commit covering all 3); Codex Connector silent-OK; CR R2 ran post-Tim-merge (R2-1 UTC-rollover-flake on `currentYearMonth()` after seed insert + R2-2 missing empty-month probe Assertion 14) — R2 fixes carried forward to PR #8 as Phase 0 (cherry-pick `0582474`). Validation pre-merge: tsc + harness 13/13 ✓ + curl ordering probe 401-401-400-200. **Phase E (Cohere/Gemini Embedding cost emission) + Phase D (cost-ledger LIMIT push-to-DB + fire-and-forget recordCost) DEFERRED to PR #8** — Phase E surfaced brief discrepancy: brief called Cohere usage "v4 reranker" but actual code uses Cohere v4 EMBEDDINGS (`cohere_client.embed(...)`); zero `rerank` call sites. PR #8 brief at `~/agent-vault/briefs/2026-05-07-pr8-cost-ledger-deferred-phases-d-e.md` includes corrected Cohere Embed v4 token-based pricing + architecture decision tree (Option A return-tuple recommended, mirrors `/grade_image_v2`). Brief: `~/agent-vault/briefs/2026-05-06-pr7-cost-ledger-hardening-additive.md`.

**PR #6 Caller-Auth-Gate + JWT Plumbing (2026-05-07 → MERGED 2026-05-07T22:10:49Z squash `bcf2eb3`):** Closed the caller-auth-gate hole left open by PR #4 + PR #5 (R1-2 endpoint 401/403 + R1-4 HUD JWT forwarding) plus the actual unblocker — JWT plumbing into 26 `OS_API_URL` fetch sites in `src/api.ts`. Phase A (Brandy): new `src/lib/apiAuth.ts::getAuthHeaders()` + `getCurrentClientToken()` accessor in `src/lib/supabase.ts` (single source of truth — supabase-js fetch wrapper unchanged); spliced into all 26 sites; mint endpoint POST left bare (bootstrap can't carry JWT). Phase B (Brandy): 401/403 hardening on `signed-url` + `cost-ledger` endpoints — `verifyClientJwtFromRequest` + `jwtAuthEnabled` gate; 401 when flag-on + missing JWT; 403 when caller's `client_id` doesn't match resource's; bootstrap-fallback when flag-off preserved. Phase G core (Brandy): harness assertions 8-10 in `_pr4-storage-and-ledger.ts` (Assertion 8 signed-URL 401, Assertion 9 cost-ledger 401, Assertion 10 cost-ledger cross-tenant 403). Phase C smoke (Karl): chrome-devtools-mcp HUD walkthrough with `VITE_JWT_AUTH_ENABLED=true` — DevTools Network tab confirmed Authorization headers on 4+ representative routes; JWT payload `{client_id: client_drift-mv, iss: os-api, exp-iat=3600}`; bootstrap-fallback verified pre-mint. **CR convergence on PR branch:** R1 1 Major (R1-1 cross-tenant check on `jwtAuthEnabled` gate) → fix accept-and-mirror. R2 1 Major (R2-1 resource-existence leak; DB lookup before auth gate) → fix auth gate FIRST then DB lookup; bogus-ID probe now returns 401 not 404. R3 1 Minor outside-diff (R3-1 storagePath null check before tenant 403) → fix tenant check before storagePath gate, signed-url only. **R4 silent-OK** at 2026-05-07T07:24:42Z UTC — "No actionable comments... 🎉" (run ID `80e1d69a-6f65-4cae-a245-53c875a42129`). Validation pre-merge: tsc + build clean (root + os-api + HUD), harness 10/10 ✓, `_phase7-multi-tenant-isolation` 5/5 (no regression). **Required env vars when flag flips on:** `JWT_AUTH_ENABLED=true` (server) + `VITE_JWT_AUTH_ENABLED=true` (HUD). Brief: `~/agent-vault/briefs/2026-05-06-pr6-caller-auth-gate-jwt-plumbing.md` (Progress Log filled with Phase A/B/G core/C + R1/R2/R3 fixes + R4 silent-OK).

**PR #5 Storage Read Hardening (migration 018, 2026-05-04 → MERGED 2026-05-06 squash `2bb016e`):** Closes the URL-as-secret read-side gap from PR #4. New `GET /api/artifacts/:artifactId/signed-url` endpoint mints 1h TTL signed URLs gated on JWT tenant match (`verifyClientJwtFromRequest` helper in `os-api/src/auth.ts` + `createArtifactSignedUrl` helper in `os-api/src/storage.ts`). HUD `useSignedArtifactUrl` hook (`src/hooks/`) — module-level Map cache with 5-min refresh buffer, cancellation-safe useEffect, no persistence. Migrated 3 HUD readers (ReviewPanel.tsx artifact previews + ShotDetailDrawer.tsx IterationThumb) from `<img src={artifact.path}>` to the hook; production-helper paths (`getProductionStillUrl` etc. in AnchorStillPanel/DeliverableTracker/ShotReshootDrawer) preserved as-is — they proxy through os-api file-serving, not Storage. Migration 018 flips `storage.buckets.public = false` for `artifacts` (single SQL UPDATE; rollback via `UPDATE storage.buckets SET public = true`). Post-flip verification: `/object/public/` returns HTTP 400 (leak closed), `/object/sign/?token=` returns HTTP 200 (HUD works), Migration 016 SELECT policy `artifacts_client_read` now active. Verification harness `_pr4-storage-and-ledger.ts` extended in-place to 7 assertions (was 5) — Assertion 6 own-tenant signed URL fetches HTTP 200 with seeded bytes; Assertion 7 cross-tenant signed-URL request returns HTTP 403. **Pre-requisite for tenant gate to engage:** os-api running with `JWT_AUTH_ENABLED=true` (bootstrap-falls-back to anonymous-allowed when off — same known-limitation pattern as `/cost-ledger`; PR #6 unifies operator-auth-gate work). Brief: `~/agent-vault/briefs/2026-05-04-pr5-storage-read-hardening.md`.
